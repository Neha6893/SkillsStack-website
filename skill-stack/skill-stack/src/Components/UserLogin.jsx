...
setToken(response.token);  // now token is just the string
...
await localStorage.setItem("user",JSON.stringify({ "name":response.user.name,
      "mail":response.user.mail,
      "phoneno":response.user.phoneno,
      "points":response.user.points,}));
    navigate("/profile");
...
else
{
  ...
  return (
    <div className="flex justify-center items-center">
      ...
      <button
        onClick={handleOpen}
        className="logbtn text-white hover:bg-green-700 px-3.5 py-0.5 m-2 font-bold rounded-sm"
      >
        Login
      </button>
             ...
             {isButtonVisible &&( <button
              ...
              {isVerifyOpen && (<button
                ...
                className={`py-2 rounded-lg w-full text-white 
                  ${otp.length < 6 ? "bg-gray-400 cursor-not-allowed" : "bg-green-500 hover:bg-green-600"}`}
              >
                Verify OTP
              </button>)}
            </form>
